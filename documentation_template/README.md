...
Links can be inserted with [link text in square brackets] followed by (link URL in parantheses).  For example, the markdown for [link to this page](./) on GitHub is: 
`[link to this page](./)`  
  
Note the use of relative paths in links.  The dot slash `./` in link path means the current directory, `../` refers to the parent directory, `../../` is one more directory up and so on.  
